...
(defun test-attack (entity &key (target-point '(0 0)) (attack-type 'physical))
  (let* ((x (car target-point))
	...
	(progn (calculate-entity-attack entity :target-point (list x y) :attack-type attack-type)
	       t)
	(if (and (creature-p entity)
		 (eq (entity-x player) x)
		 (eq (entity-y player) y))
	    (progn (calculate-entity-attack entity :attack-type attack-type)
		   t)
	    (progn (if (eq (aref item-array y x) 0)
		       (push-message (concatenate 'string "You strike the " object " for no apparent reason."))
		       (if (not (creature-p entity))
			   (push-message (concatenate 'string "You're attacking the " (item-name (aref item-array y x)) "!?!? This is the worst thing since my son."))))
		   nil))
	)))

(defun calculate-entity-attack (entity &key target-point (attack-type 'physical))
  (let* ((target (if (player-character-p entity)
	 ...
	 (defense-power (+ (entity-defense target) (entity-defense-mod target) (if (item-p (entity-armor target))
										   ...
										   0)))
	 (attack-chance (if (< (+ (entity-dodge target) (entity-dodge-mod target)) (+ (random 50) 50))
			    t
			    nil))
	 )
    (if attack-chance
	(if (and (> (- attack-power defense-power) 0)
		 (not (eq (- attack-power defense-power) 0)))
	...
	(if (player-character-p entity)
	    (if (eq attack-type 'physical)
		(push-message "Swing and a miss.")
		(push-message "Flew by the target's head."))
	    (push-message (concatenate 'string "The " (entity-name entity) " missed by a margin.")))
	)
...
(defun test-ranged-attack (entity &key target-point)
  ...
  (let* ((target-aquired (path-search (list (cadr (assoc :x (entity-position entity))) 
					    (cadr (assoc :y (entity-position entity))))
				      target-point))
	 )
    ;;;;if x is bigger than y, then loop x by number, otherwise, loop y by number
    (if target-aquired
	(let ((terrain-object (if (eq (car target-aquired) 'terrain)
				  (cond ((string= (aref main-map y x) (cadr (assoc 'tree-trunk symbols-list)))
					...
					((string= (aref main-map y x) (cadr (assoc 'floor symbols-list)))
					 "air")))))
	  (case (car target-aquired)
	    (terrain (push-message (concatenate 'string "And you hit a " terrain-object ".")))
	    (enemy (test-attack entity :target-point target-point :attack-type 'ranged))
	    (item (push-message "You hit an...item. Why are you so against inanimate objects?"))
	    ))
	(push-message "The projectile flies through the air.")
	)))
...
(defun path-search (start-point target-point)
  ;;;;start-x end-x, start-y end-y
  (let* ((end-x (car target-point))
	 (end-y (cadr target-point))
	 (start-x (car start-point))
	 (start-y (cadr start-point))
	 (line (list (- end-x start-x) (- end-y start-y)))
	 (is-bigger (if (> (abs (car line)) (abs (cadr line)))
			'x
			'y))
			   #|(if (eq (car line) 0)
			'y
